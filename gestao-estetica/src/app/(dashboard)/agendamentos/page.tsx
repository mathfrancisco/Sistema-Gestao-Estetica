...
const AgendamentosPage: React.FC = () => {
    ...
    const {
        ...
        autoFetch: false, // Vamos controlar manualmente
        initialLimit: 50 // Carregar mais itens para facilitar filtragem
    })
    ...
    const getAppointmentStats = () => {
        ...
        return {
            ...
            unsynced: appointmentsWithDetails.filter(apt => !apt.calendar_synced).length
        }
    }
                                    ...
                                    <CardContent className="p-4 lg:p-6">
                                        <div className="flex flex-wrap gap-3">
                                            <Button
                                                ...
                                                onClick={() => {
                                                    // Lógica para sincronizar todos os agendamentos não sincronizados
                                                    const unsyncedCount = filteredAppointments.filter(apt => !apt.calendar_synced).length
                                                    if (unsyncedCount > 0) {
                                                        toast.info(`${unsyncedCount} agendamentos serão sincronizados`)
                                                    } else {
